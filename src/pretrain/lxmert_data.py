...
class InputExample(object):
    ...
    def __init__(self, uid, sent, visual_feats=None, obj_labels=None, attr_labels=None, is_matched=None, label=None):
        ...
        self.is_matched = is_matched  # visual과 obj가 matching되었는지
        self.label = label
...
class LXMERTTorchDataset(Dataset):
    def __init__(self, dataset: LXMERTDataset, topk=-1):
        ...
        # dataset flatten (one sent + one image entry로)
        self.data = []
        ...
        print("Use %d data in torch data set" % len(self.data))

    def __len__(self):
        return len(self.data)

    def random_feat(self):
        """ dataset에서 random obj feat 가져오기 """
    ...
    def __getitem__(self, item: int):
        ...
        attr_labels = img_info['attr_id'].copy()
        attr_confs = img_info['attr_conf'].copy()
        assert obj_num == len(boxes) == len(feats)

        # boxes를 0~1로 normalize
        ...
        # matched loss 계산 시, sentence를 다른 이미지에 해당하는 sentence로 대체
        is_matched = 1
        sent = datum['sent']
...
class LXMERTEvaluator:
    ...
    def evaluate(self, uid2ans, dict, pprint=False):
        ...
        for uid, ans in uid2ans.items():
            if uid not in self.uid2datum:  # labeled data가 아님
                continue
