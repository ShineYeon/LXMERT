...
def get_tuple(splits: str, bs: int, shuffle=False, drop_last=False, topk=-1) -> DataTuple:
    # pre-training 중 어떤 QA dataset을 사용할지 결정
    # Options: vqa, gqa, visual17w
    # Note: visual17w는 vgqa의 한 부분임.
    qa_sets = args.qa_sets
    if qa_sets is not None:
        qa_sets = set(qa_set.lower().strip() for qa_set in qa_sets.split(","))

    # dataset, data loader, evaluator 로드하기
...
def random_word(tokens, tokenizer):
    """
    LM task를 위한 random token들을 원본 BERT 논문에 나온 확률과 동일하게 만듦.
    :param tokens: list of str, tokenized sentence
    :param tokenizer: Tokenizer, object used for tokenization
    ...
    for i, token in enumerate(tokens):
        ...
        if prob < ratio:
            ...
            # 80%로 랜덤하게 토큰을 마스킹 토큰으로 변경
            if prob < 0.8:
                tokens[i] = "[MASK]"

            # 10%로 랜덤하게 토큰을 랜덤 토큰으로 변경
            if prob < 0.9:
                tokens[i] = random.choice(list(tokenizer.vocab.items()))[0]

            # 나머지 10%는 랜덤하게 현재 토큰유지

            # 현재 토큰을 output에 append
            try:
                output_label.append(tokenizer.vocab[token])
            except KeyError:
                # unknown words를 위해서임 (BPE vocab에 나타나지 않음)
...
def random_feat(feats):
    ...
    for i in range(len(feats)):
        ...
        if prob < args.obj_mask_rate:
            ...
            # 80%로 랜덤하게 토큰을 zero feat으로 변경
            if prob < 0.8:
                mask_feats[i, :] = 0.

            # 10%로 랜덤하게 토큰을 random feat으로 변경
            elif prob < 0.9:
                mask_feats[i, :] = train_tuple.torchdset.random_feat()

            # 10%는 랜덤하게 현재 feat 유지
...
def convert_example_to_features(example: InputExample, max_seq_length, tokenizer) -> InputFeatures:
    """
    raw sample(tokenize된 string으로 구성된 sentence of pair)을 proper training sample로 변환 (ID, LM label, input_mask, CLS 및 SEP 토큰)
    :param example: InputExample, string과 is_next label로 sentence input 구성
    :param max_seq_length: int, sequence의 최대 길이
    :param tokenizer: Tokenizer
    :return: InputFeatures, 모든 input과 한 샘플의 label을 ID로써 포함 (모델 training에 사용)
    """
    tokens = tokenizer.tokenize(example.sent.strip())

    # [CLS]토큰과 [SEP]토큰을 "-2"로 집계
    ...
    # lm label과 CLS, SEP, SEP에 대한 집계를 concat
    masked_tokens = ['[CLS]'] + masked_tokens + ['[SEP]']
    input_ids = tokenizer.convert_tokens_to_ids(masked_tokens)

    # Mask $ Segment Word
    lm_labels_ids = ([-1] + masked_label + [-1])
    ...
    while len(input_ids) < max_seq_length:
        ...
        lm_labels_ids.append(-1)

    assert len(input_ids) == max_seq_length
    assert len(input_mask) == max_seq_length
    assert len(segment_ids) == max_seq_length
    assert len(lm_labels_ids) == max_seq_length
...
LOSSES_NAME = ('Masked_LM', 'Matched', 'Obj', 'Attr', 'Feat', 'QA')
...
class LXMERT:
    def __init__(self, max_seq_length):
        ...
        if args.from_scratch:
            ...
            self.model.apply(self.model.ini_bert_weights)
    ...
    def train(self, train_tuple: DataTuple, eval_tuple: DataTuple):
            ...
            for batch in tqdm(train_ld, total=len(train_ld)):
                ...
                if args.task_qa:
                    ...
                    for datum, l in zip(batch, label.cpu().numpy()):
                        uid = datum.uid
                        ans = train_tuple.dataset.answer_table.id2ans(1)
                        uid2ans[uid] = ans

            print("Teh training loss for Epoch %d is #0.4f" % (epoch, total_loss / batch_per_epoch))
        ...
        def evaluate_epoch(self, eval_tuple: DataTuple, iters: int = -1):
            ...
            losses_str = "The losses are "
            for name, loss in zip(LOSSES_NAME, total_loss / len(eval_ld)):
